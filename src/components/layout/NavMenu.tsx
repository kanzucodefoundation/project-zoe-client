...
const routes = [
  {
    name: "Dashboard",
    requiredRoles: [
      appRoles.roleCrmView,
      appRoles.roleAuthUserView,
      appRoles.roleAuthUserEdit,
      appRoles.roleAuthGroupView,
      appRoles.roleAuthGroupEdit,
      appRoles.roleTagView,
      appRoles.roleTagEdit,
      appRoles.roleGroupView,
      appRoles.roleGroupEdit,
      appRoles.roleCrmEdit,
      appRoles.roleVolunteer,
      appRoles.roleTeamLead,
    ],
    ...
    <div style={{ backgroundColor: navBackgroundColor }}>
      ...
      <List style={{ paddingTop: 0 }}>
        {routes.map((it) => {
          if (
            it.requiredRoles &&
            it.requiredRoles.length > 0 &&
            hasAnyRole(user, it.requiredRoles)
          ) {
            const Icon = it.icon;
            if (it.items) {
              return (
                <Fragment key={it.name}>
                  ...
                  <Collapse
                    in={
                      open[it.name] || isSelected(it.name.toLocaleLowerCase())
                    }
                    timeout="auto"
                    unmountOnExit
                  >
