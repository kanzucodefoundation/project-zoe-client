...
export default function reducer(state = initialState, action: any) {
  switch (action.type) {
    ...
    case crmConstants.crmDeleteEmail: {
      ...
      if (state.selected) {
        const { emails, ...rest } = state.selected;
        const selected: IContact = {
          ...rest,
          emails: emails.filter((it) => it.id !== id),
        };
        return { ...state, selected };
      }
      return state;
    ...
    case crmConstants.crmDeletePhone: {
      ...
      if (state.selected) {
        ...
        const selected: IContact = {
          ...rest,
          phones: phones.filter((it) => it.id !== id),
        };
        return { ...state, selected };
      }
      return state;
    ...
    case crmConstants.crmDeleteIdentification: {
      ...
      if (state.selected) {
        ...
        const selected: IContact = {
          ...rest,
          identifications: identifications.filter((it) => it.id !== id),
        };
        return { ...state, selected };
      }
      return state;
    ...
    case crmConstants.crmDeleteAddress: {
      ...
      if (state.selected) {
        ...
        const selected: IContact = {
          ...rest,
          addresses: addresses.filter((it) => it.id !== id),
        };
        return { ...state, selected };
      }
      return state;
