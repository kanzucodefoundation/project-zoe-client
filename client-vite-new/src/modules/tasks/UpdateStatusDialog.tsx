...
import {
  TaskStatus, STATUS_LABELS, NEXT_STATUS_OPTIONS, CLOSED_STATUSES,
  type Task,
} from '../../utils/types';
...
function getValidationSchema(status: TaskStatus) {
  if (status === TaskStatus.ATTENDED_FELLOWSHIP || status === TaskStatus.JOINED_SERVING_TEAM) {
    return Yup.object({
...
export default function UpdateStatusDialog({ open, task, onClose, onSuccess }: Props) {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.lazy((values: any) => getValidationSchema(values.status)),
    onSubmit: (values) => {
      ...
      if (
        ...
        data = {
          ...
          ...(values.baptismLocation && { baptismLocation: values.baptismLocation }),
          ...(values.baptismOfficiant && { baptismOfficiant: values.baptismOfficiant }),
        };
      }
      updateStatus.mutate(
        { id: task.id, data },
  ...
  useEffect(() => {
    ...
    if (s === TaskStatus.ATTENDED_FELLOWSHIP || s === TaskStatus.JOINED_SERVING_TEAM) {
      const purpose = s === TaskStatus.ATTENDED_FELLOWSHIP ? 'fellowship' : 'serving_team';
      ajax
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 1 }}>
            <FormControl>
              ...
              >
                {availableStatuses.map((s) => (
                  <FormControlLabel key={s} value={s} control={<Radio />} label={STATUS_LABELS[s]} />
                ))}
              </RadioGroup>
            </FormControl>
            ...
            {needsGroup && (
              <>
                <Autocomplete
                  ...
                  onChange={(_, val) => formik.setFieldValue('groupId', val?.id ?? null)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      ...
                      helperText={formik.touched.groupId && (formik.errors.groupId as string)}
                    />
                  )}
                />
                <DatePicker
            ...
            {selectedStatus === TaskStatus.MATCHED_TO_FELLOWSHIP && (
              <Alert severity="info">
                This will record that the contact has been matched to a fellowship.
              </Alert>
            )}
          </Stack>
        </DialogContent>
